...
it("TEST DOCUMENT GENERATOR", () => {
    ...
    document.createEntry().setOriginal("American homeowners mow their lawns.");
    document.createEntry().setOriginal("Dog owners pick up after their dogs.");
    document.createEntry().setOriginal("The Commission may appoint its own Secretary and staff.");
    document.createEntry().setOriginal("For the purposes of this Act, working hours means time when the employee is at the disposal of the employer.");
    document.createEntry().setOriginal("Normal working hours must not exceed nine hours per 24 hours and 40 hours per seven days.");
    document.createEntry().setOriginal("The Ministry may issue regulations with further provisions concerning the activities of the committee, including provisions concerning procedure and concerning the duty of secrecy for members of the committee.");
    document.createEntry().setOriginal("“Contracting Government” means any Government which has deposited an instrument of ratification or has given notice of adherence to this Convention.");
    document.createEntry().setOriginal("Night work is not permitted unless necessitated by the nature of the work.");
    document.createEntry().setOriginal("The employer and the employee may agree in writing that overtime hours shall wholly or partly be taken out as off-duty time on agreed dates.");
    document.createEntry().setOriginal("The Commission may set up, from among its own members and experts or advisers, such committees as it considers desirable to perform such functions as it may authorize.");
    document.createEntry().setOriginal("All male U.S. citizens, 18 years of age and older, must register with the Selective Service by filling out a form at the U.S. Post Office or else face arrest for evading registration.");
    document.createEntry().setOriginal("Certified organic farmers must not apply synthetic chemicals to crops at any time once organic certification is conferred, or else certifier will administer official notice of noncompliance and revoke or suspend certification of farmer.");
    document.createEntry().setOriginal("Employees who have children in their care are entitled to leave of absence:\na) when necessary to attend a sick child,\nb) if a child shall be accompanied to a medical examination or other follow-up in connection with sickness, or\nc) if the person responsible for the daily childcare is sick or has leave of absence pursuant to this section owing to another child.");
    document.createEntry().setOriginal("Such notification shall provide:\n(1) A description of each noncompliance;\n(2) The facts upon which the notification of noncompliance is based; and\n(3) The date by which the certifying agent must rebut or correct each noncompliance and submit supporting documentation of each correction when correction is possible.");
    document.createEntry().setOriginal("Drivers must hand their driver’s license to the police officer when stopped in traffic control, or else the police officer must enforce this under any circumstances and, depending on severity, must either fine the driver or arrest him, or else internal investigators must follow up on this issue in any case.\n");
    //console.log(JSON.stringify(document, null, 2));
});

//------------------------------------------------------------------------------

/** Generator for the dev test document */
it("DEV TEST DOCUMENT GENERATOR", () => {
    // Setup
    const document = new Document("Policy Coding Test", "", 1);
