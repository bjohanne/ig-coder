...
const StatementEditor = (props: IProps) => {
    ...
    */
	...
	const [hasOrElse, setHasOrElse] = useState(getHasOrElse());
	...
	const changeHasOrElse = (e: React.ChangeEvent<HTMLInputElement>) => {
        setHasOrElse(e.target.checked);
    }
    ...
    const entryTextChanged = (e: React.FormEvent<HTMLTextAreaElement>) => {
        let { value } = e.currentTarget; // extra .value?
        setEntryText(value);
    }
        ...
        <div className="card-body entry-editor">
            <div className="row">
				...
				<div className="col-md-10 d-flex justify-content-start align-items-center">
					...
					<Toggle
                        ...
                        defaultChecked={hasOrElse}
                        aria-labelledby='has-sanction-label'
                        onChange={changeHasOrElse}
						className="ml-5" />
			...
			<div className="row">
				<div className="col-md-12">
					<textarea name="node-entry-content" onChange={entryTextChanged} className="form-control input-lg mb-3" value={entryText}/>
				</div>
			</div>
