...
class DataAccess(object):
        ...
        for item in nodes:
            ...
            if "children" in item:
                for child in item["children"]:
                    ...
                    statements\
                        .append("MERGE (Document{4}{0})-[:child_of]->(Document{4}{1})"
                                .format(child["id"], item["id"], child["nodeType"], item["nodeType"], item["document"]))
                del item["children"]
    ...
    def get_document(self, document_id):
        q = """ MATCH (n) WHERE n.document = "{0}" RETURN n """.format(document_id)
        with self._driver.session() as session:
